...
function buildPricePlanMap(): Record<string, PriceMapping> {
  ...
  const teamId = process.env.STRIPE_PRICE_TEAM;
  if (proId) map[proId] = { plan: "pro", profile_limit: 100, cadence: "daily" };
  if (teamId) map[teamId] = { plan: "team", profile_limit: 1000, cadence: "hourly" };
  return map;
}
