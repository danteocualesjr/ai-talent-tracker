...
export function isAllowedCheckoutPrice(priceId: string): boolean {
  const configured = [process.env.STRIPE_PRICE_PRO, process.env.STRIPE_PRICE_TEAM].filter(Boolean) as string[];
  if (configured.length > 0) return configured.includes(priceId);
