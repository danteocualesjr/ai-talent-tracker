...
export function isAllowedPriceId(priceId: string): boolean {
  ...
  // Reject placeholder keys when real Stripe price env vars are unset.
  if (priceId === "price_pro" && !process.env.STRIPE_PRICE_PRO) return false;
  if (priceId === "price_team" && !process.env.STRIPE_PRICE_TEAM) return false;
  return true;
}
