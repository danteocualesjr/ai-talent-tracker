...
export const PRICE_PLAN_MAP: Record<string, { plan: Plan; profile_limit: number; cadence: "weekly" | "daily" | "hourly" }> =
  Object.fromEntries(
    [
      [process.env.STRIPE_PRICE_PRO, { plan: "pro" as const, profile_limit: 100, cadence: "daily" as const }],
      [process.env.STRIPE_PRICE_TEAM, { plan: "team" as const, profile_limit: 1000, cadence: "hourly" as const }],
    ].filter(([id]) => Boolean(id)) as [string, { plan: Plan; profile_limit: number; cadence: "weekly" | "daily" | "hourly" }][],
  );
