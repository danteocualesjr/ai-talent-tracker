...
export async function listOrgProfiles(orgId: string): Promise<(Profile & { watchlist_id: string })[]> {
  ...
  for (const row of (data ?? []) as unknown as Array<{ watchlist_id: string; profiles: Profile }>) {
    const p = row.profiles as Profile;
    byId.set(p.id, { ...p, watchlist_id: row.watchlist_id });
  }
