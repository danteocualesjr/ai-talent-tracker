...
export async function listOrgProfiles(orgId: string): Promise<(Profile & { watchlist_id: string })[]> {
  ...
  return ((data ?? []) as unknown as Array<{ watchlist_id: string; profiles: Profile | null }>)
    .filter((r): r is { watchlist_id: string; profiles: Profile } => Boolean(r.profiles))
    .map((r) => ({
      ...r.profiles,
...
export async function getOrgEvents(orgId: string, limit = 50): Promise<(EventRow & { profile: Profile })[]> {
  ...
  return ((data ?? []) as unknown as (EventRow & { profile: Profile | null })[]).filter(
    (e): e is EventRow & { profile: Profile } => Boolean(e.profile),
  );
}
...
export async function isProfileWatchedByOrg(orgId: string, profileId: string): Promise<boolean> {
