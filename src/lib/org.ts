...
export async function ensureOrgForUser(userId: string, email: string | null): Promise<Organization> {
  ...
  const { data: org, error } = await db
    ...
    .single();
  if (error) {
    // Concurrent first-login requests can race on slug uniqueness.
    if (error.code === "23505") {
      const retry = await db
        ...
        .limit(1)
        .maybeSingle();
      if (retry.data && (retry.data as { organizations?: unknown }).organizations) {
        const o = (retry.data as { organizations: unknown }).organizations;
        return Array.isArray(o) ? (o[0] as Organization) : (o as Organization);
      }
    }
    throw error;
