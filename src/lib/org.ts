...
export async function ensureOrgForUser(userId: string, email: string | null): Promise<Organization> {
  ...
  if (error) {
    // Concurrent first-login requests can race on slug; return the org another request created.
    const retry = await db
      ...
      .limit(1)
      .maybeSingle();
    if (retry && (retry as unknown as { organizations?: unknown }).organizations) {
      const o = (retry as unknown as { organizations: unknown }).organizations;
      return Array.isArray(o) ? (o[0] as Organization) : (o as Organization);
    }
    throw error;
  }
