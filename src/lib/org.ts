...
export async function ensureOrgForUser(userId: string, email: string | null): Promise<Organization> {
  ...
  const { data: org, error } = await db
    ...
    .single();
  if (error) {
    // Concurrent first-logins can race on slug; re-fetch the membership created by the winner.
    if (error.code === "23505") {
      const { data: retry } = await db
        ...
        .limit(1)
        .maybeSingle();
      if (retry && (retry as { organizations?: unknown }).organizations) {
        const o = (retry as { organizations: unknown }).organizations;
        return Array.isArray(o) ? (o[0] as Organization) : (o as Organization);
      }
    }
    throw error;
