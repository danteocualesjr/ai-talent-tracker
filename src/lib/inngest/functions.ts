...
export const scheduleRefreshes = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const due = await step.run("find-due-profiles", async () => {
      ...
      const [{ data: overdue, error: overdueErr }, { data: neverSynced, error: neverErr }] = await Promise.all([
        db.from("profiles").select("id").eq("is_opted_out", false).lte("next_sync_at", now).limit(500),
        db.from("profiles").select("id").eq("is_opted_out", false).is("next_sync_at", null).limit(500),
      ]);
      if (overdueErr) throw overdueErr;
      if (neverErr) throw neverErr;
      const ids = new Set<string>();
      for (const row of [...(overdue ?? []), ...(neverSynced ?? [])]) ids.add((row as { id: string }).id);
...
async function nextSyncAt(
  ...
  for (const row of watchers ?? []) {
    const watchlists = (row as { watchlists?: { organizations?: { refresh_cadence?: RefreshCadence } | { refresh_cadence?: RefreshCadence }[] } }).watchlists;
    const orgs = watchlists?.organizations;
    const cadence = Array.isArray(orgs) ? orgs[0]?.refresh_cadence : orgs?.refresh_cadence;
