...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const isBaselineSync = !profile.last_synced_at;
...
const CADENCE_HOURS: Record<RefreshCadence, number> = { weekly: 168, daily: 24, hourly: 1 };
...
async function resolveNextSyncAt(
  ...
  for (const row of watchers ?? []) {
    const wl = row.watchlists as { organizations?: { refresh_cadence?: RefreshCadence } | null } | null;
