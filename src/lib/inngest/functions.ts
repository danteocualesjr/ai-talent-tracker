...
export const scheduleRefreshes = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const due = await step.run("find-due-profiles", async () => {
      ...
      const watched = (q: ReturnType<typeof db.from>) =>
        q.select("id, watchlist_profiles!inner(profile_id)").eq("is_opted_out", false).limit(500);

      const [dueRes, nullRes] = await Promise.all([
        watched(db.from("profiles")).lte("next_sync_at", iso),
        watched(db.from("profiles")).is("next_sync_at", null),
      ]);
      if (dueRes.error) throw dueRes.error;
      if (nullRes.error) throw nullRes.error;

      const ids = new Set<string>();
      for (const row of [...(dueRes.data ?? []), ...(nullRes.data ?? [])] as { id: string }[]) {
        ids.add(row.id);
      }
...
async function computeNextSyncAt(db: AdminDb, profileId: string, profile: Profile): Promise<string> {
  ...
  for (const row of watchers ?? []) {
    ...
    if (org?.refresh_cadence) cadenceHours.push(cadenceToHours(org.refresh_cadence));
  }

  let hours = cadenceHours.length > 0 ? Math.min(...cadenceHours) : 24 * 7;
