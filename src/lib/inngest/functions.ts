...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const { priorSnapshotCount, stored } = await step.run("store-snapshot", async () => {
      const { count: priorSnapshotCount } = await db
      ...
      const { data: existing } = await db
        ...
        .maybeSingle();
      if (existing) return { priorSnapshotCount: priorSnapshotCount ?? 0, stored: null };
      ...
      const { data, error } = await db
        ...
        .single();
      if (error) throw error;
      return { priorSnapshotCount: priorSnapshotCount ?? 0, stored: data as ProfileSnapshot };
    });
    ...
    // First snapshot establishes a baseline — no diff/events yet.
    if (priorSnapshotCount === 0) return { changed: true, eventCreated: false, baseline: true };
    ...
    new Set(
      ((watchers ?? []) as Array<{ watchlists?: { org_id?: string } | { org_id?: string }[] | null }>)
        .map((w) => {
          ...
          return Array.isArray(wl) ? wl[0]?.org_id : wl.org_id;
        })
