  ...
  async ({ event, step }) => {
    ...
    const stored = await step.run("store-snapshot", async () => {
      ...
      const { data, error } = await db
        ...
        .single();
      if (error) {
        // Concurrent refresh with identical content — treat as no-op.
    ...
    if (isFirstSync) return { changed: false, firstSync: true };
...
async function nextSyncAt(db: ReturnType<typeof createAdminClient>, profile: Profile): Promise<string> {
  ...
  let orgCadenceHours = CADENCE_HOURS.weekly;
  for (const row of watchers ?? []) {
    ...
    if (cadence && CADENCE_HOURS[cadence] < orgCadenceHours) {
      orgCadenceHours = CADENCE_HOURS[cadence];
    }
  }

  const hours =
    profile.status === "left" || profile.status === "stealth"
      ? Math.min(6, orgCadenceHours)
      : orgCadenceHours;
