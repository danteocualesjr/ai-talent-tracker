...
export const scheduleRefreshes = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const due = await step.run("find-due-profiles", async () => {
      ...
      const [{ data: nullDue, error: nullErr }, { data: pastDue, error: pastErr }] = await Promise.all([
      ...
      for (const row of [...(nullDue ?? []), ...(pastDue ?? [])] as { id: string }[]) {
        ids.add(row.id);
      }
...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const preserveNullable = provider.name === "manual";
    ...
    await step.run("touch-profile", async () => {
      ...
      const { error } = await db
        ...
        .update({
          ...
          current_company: preserveNullable
            ? (fetched.current_company ?? profile.current_company)
            : fetched.current_company,
          current_title: preserveNullable
            ? (fetched.current_title ?? profile.current_title)
            : fetched.current_title,
    ...
    const nextForDiff: ProviderProfile = preserveNullable
      ? {
          ...fetched,
      ...
      : fetched;
    const diffs = diffProfiles(prev, nextForDiff);
    ...
    let classification = classifyByRules(
      ...
      { current_company: nextForDiff.current_company, headline: nextForDiff.headline },
    );
    ...
    if (!classification || classification.confidence < 0.6) {
      const llm = await step.run("llm-classify", async () =>
        classifyWithLLM({
          ...
          next: nextForDiff as unknown as Record<string, string | null>,
        }),
      );
