...
async function computeNextSyncAt(
  ...
  for (const row of data ?? []) {
    const wl = (row as { watchlists?: { organizations?: { refresh_cadence?: RefreshCadence } | { refresh_cadence?: RefreshCadence }[] | null } | null }).watchlists;
    const org = wl && !Array.isArray(wl) ? wl.organizations : undefined;
    const cadence = (Array.isArray(org) ? org[0] : org)?.refresh_cadence;
