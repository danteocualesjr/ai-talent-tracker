...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // First fetch establishes baseline data — skip spurious change events.
...
async function nextSyncAt(
  ...
  const cadences = ((watchers ?? []) as Array<{ watchlists?: unknown }>)
    .map((w) => {
      ...
      return (o as { refresh_cadence?: RefreshCadence })?.refresh_cadence;
    })
