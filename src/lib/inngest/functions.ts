...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const priorSnapshotCount = await step.run("count-prior-snapshots", async () => {
    ...
    // First snapshot establishes baseline only — no change events or alerts.
    if (priorSnapshotCount === 0) return { changed: false, baseline: true };
...
async function resolveNextSyncAt(db: AdminDb, profileId: string, profile: Profile): Promise<string> {
  const leftOrStealth = profile.status === "left" || profile.status === "stealth";
  let hours = leftOrStealth ? 6 : 168;
  ...
  for (const row of watchers ?? []) {
    const wl = (row as { watchlists?: { organizations?: { refresh_cadence?: string } | null } | null })
      .watchlists;
    const cadence = wl?.organizations?.refresh_cadence;
    if (!cadence) continue;
    const cadenceHours = CADENCE_HOURS[cadence] ?? 168;
    hours = Math.min(hours, cadenceHours);
  }
