...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const isBaseline = await step.run("is-baseline-snapshot", async () => {
    ...
    if (classification.type === "other" && classification.confidence < 0.5) {
...
async function computeNextSyncAt(
  ...
  for (const row of watchers ?? []) {
    const cadence = cadenceFromWatcher(row);
...
function cadenceFromWatcher(row: unknown): RefreshCadence | undefined {
  const wl = (row as { watchlists?: { organizations?: { refresh_cadence?: RefreshCadence } | null } | { organizations?: { refresh_cadence?: RefreshCadence } | null }[] | null }).watchlists;
