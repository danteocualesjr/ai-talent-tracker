...
async function nextSyncAt(
  ...
  for (const row of (data ?? []) as Array<{ watchlists?: unknown }>) {
    const cadence = extractRefreshCadence(row.watchlists);
...
function extractRefreshCadence(watchlists: unknown): RefreshCadence | null {
  const rows = Array.isArray(watchlists) ? watchlists : watchlists ? [watchlists] : [];
