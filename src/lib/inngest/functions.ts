...
async function resolveNextSyncAt(
  ...
  for (const row of watchers ?? []) {
    const wl = (row as { watchlists?: { organizations?: { refresh_cadence?: RefreshCadence } } }).watchlists;
    const cadence = wl?.organizations?.refresh_cadence;
    if (cadence && CADENCE_HOURS[cadence] < fastest) {
      fastest = CADENCE_HOURS[cadence];
    }
  }

  // Left/stealth profiles get checked more often regardless of plan.
  if (profile.status === "left" || profile.status === "stealth") {
    fastest = Math.min(fastest, 6);
  }
