  ...
  async ({ step }) => {
    ...
    const due = await step.run("find-due-profiles", async () => {
      ...
      const { data, error } = await db
        .from("profiles")
        .select("id")
        .or(`next_sync_at.is.null,next_sync_at.lte."${iso}"`)
...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Skip event creation on the baseline snapshot (no prior data to diff against).
    const { count: snapshotCount, error: countErr } = await db
      .from("profile_snapshots")
