...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Skip event creation on the first snapshot — no prior baseline to diff against.
    const priorCount = await step.run("count-prior-snapshots", async () => {
    ...
    if (priorCount <= 1) return { changed: true, eventCreated: false, baseline: true };
