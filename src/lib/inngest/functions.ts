...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const stored = await step.run("store-snapshot", async () => {
      ...
      const isBaseline = (priorCount ?? 0) === 0;
      ...
      const { data: existing } = await db
        ...
        .maybeSingle();
      if (existing) return { snapshot: null, isBaseline };
      ...
      const { data, error } = await db
        ...
        .single();
      if (error) {
        if (error.code === "23505") return { snapshot: null, isBaseline };
        throw error;
      }
    ...
    if (stored.isBaseline) return { changed: false, baseline: true };
...
async function nextSyncAt(
  ...
  if (orgIds.length > 0) {
    ...
    const cadences = ((orgs ?? []) as { refresh_cadence: RefreshCadence }[]).map((o) =>
      cadenceToHours(o.refresh_cadence),
    );
    if (cadences.length > 0) hours = Math.min(...cadences);
  }
