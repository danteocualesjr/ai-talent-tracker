...
function nextSyncAt(
  ...
  return resolveNextSyncAt(db, profileId, profile);
}
...
async function resolveNextSyncAt(
  ...
  for (const row of data ?? []) {
    const wl = (row as { watchlists?: { organizations?: { refresh_cadence?: string } | { refresh_cadence?: string }[] } | { organizations?: { refresh_cadence?: string } | { refresh_cadence?: string }[] }[] }).watchlists;
