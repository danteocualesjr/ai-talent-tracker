...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // First sync establishes a baseline; skip diffing to avoid spurious alerts.
    if (profile.last_synced_at === null) return { changed: true, eventCreated: false, baseline: true };
