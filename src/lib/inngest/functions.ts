...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Skip event creation on the first sync — there is no meaningful prior state.
    if (!profile.last_synced_at) return { changed: true, eventCreated: false, firstSync: true };
    ...
    if (!classification || classification.confidence < 0.6) {
      const llm = await step.run("llm-classify", async () => {
        try {
          ...
          console.warn("[llm-classify] failed, using rule-based result", e);
          return null;
        }
      });
...
async function computeNextSyncAt(
  ...
  if (orgIds.length > 0) {
    ...
    cadences = ((orgs ?? []) as { refresh_cadence: RefreshCadence }[]).map((o) => o.refresh_cadence);
  }
