...
export const scheduleRefreshes = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const due = await step.run("find-due-profiles", async () => {
      ...
      return (data ?? []).map((p) => ({ id: (p as { id: string }).id }));
    });
...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Skip event creation on the first sync — baseline snapshot only.
    if (!profile.last_synced_at) return { changed: true, eventCreated: false, initialSync: true };
