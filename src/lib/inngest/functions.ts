...
export const scheduleRefreshes = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const due = await step.run("find-due-profiles", async () => {
      ...
      const [{ data: overdue, error: overdueErr }, { data: neverSynced, error: neverErr }] =
        await Promise.all([
      ...
      for (const row of [...(overdue ?? []), ...(neverSynced ?? [])]) {
...
async function nextSyncAt(
  ...
  for (const row of watchers ?? []) {
    const wl = row.watchlists as { organizations?: { refresh_cadence?: string } | { refresh_cadence?: string }[] } | null;
