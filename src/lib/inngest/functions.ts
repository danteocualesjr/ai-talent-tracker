...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const hadPriorSnapshot = await step.run("check-prior-snapshots", async () => {
    ...
    // First snapshot establishes baseline — not a profile change.
...
async function nextSyncAt(db: ReturnType<typeof createAdminClient>, profile: Profile): Promise<string> {
  ...
  if (orgIds.length > 0) {
    ...
    for (const org of orgs ?? []) {
      const hours = cadenceHours((org as { refresh_cadence: RefreshCadence }).refresh_cadence);
      if (hours < baseHours) baseHours = hours;
    }
  }

  if (profile.status === "left" || profile.status === "stealth") {
    baseHours = Math.min(baseHours, 6);
  }

  return new Date(Date.now() + baseHours * 60 * 60 * 1000).toISOString();
}
