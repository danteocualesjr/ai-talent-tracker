...
export const scheduleRefreshes = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const due = await step.run("find-due-profiles", async () => {
      ...
      const [byTime, byNull] = await Promise.all([
      ...
      if (byTime.error) throw byTime.error;
      if (byNull.error) throw byNull.error;
      const ids = new Set<string>();
      for (const row of [...(byTime.data ?? []), ...(byNull.data ?? [])] as { id: string }[]) {
        ids.add(row.id);
      }
  ...
  async ({ event, step }) => {
    ...
    const stored = await step.run("store-snapshot", async () => {
      ...
      return {
        ...
        isFirstBaseline: (priorCount ?? 0) === 0,
      };
    });
    ...
    if (stored.isFirstBaseline) return { changed: true, eventCreated: false, reason: "baseline" };
