...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const hadPriorSnapshot = await step.run("count-prior-snapshots", async () => {
    ...
    // First snapshot is baseline only — avoid spurious change events on watchlist add.
    if (!hadPriorSnapshot) return { changed: false, baseline: true };
...
async function computeNextSyncAt(
  ...
  for (const row of watchers ?? []) {
    const wl = (row as { watchlists?: { organizations?: Organization | Organization[] | null } | null }).watchlists;
    if (!wl) continue;
    const org = Array.isArray(wl.organizations) ? wl.organizations[0] : wl.organizations;
    if (!org?.refresh_cadence) continue;
    hours = Math.min(hours, CADENCE_HOURS[org.refresh_cadence] ?? hours);
  }
