...
async function nextSyncAt(
  ...
  let hours = 168;
  for (const cadence of extractWatcherCadences(watchers)) {
    hours = Math.min(hours, cadenceToHours(cadence));
  }
...
function extractWatcherCadences(rows: unknown): RefreshCadence[] {
  ...
  for (const row of list) {
    ...
    for (const w of watchlists) {
      const org = (w as { organizations?: unknown }).organizations;
      const orgs = Array.isArray(org) ? org : org ? [org] : [];
      for (const o of orgs) {
        const c = (o as { refresh_cadence?: RefreshCadence }).refresh_cadence;
