...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const isBaselineSync = await step.run("is-baseline-sync", async () => {
    ...
    if (isBaselineSync) return { changed: true, eventCreated: false };
...
async function nextSyncAt(
  ...
  const cadences = ((watchers ?? []) as Array<{ watchlists?: { organizations?: { refresh_cadence?: RefreshCadence } | null } | null }>)
    .map((w) => w.watchlists?.organizations?.refresh_cadence)
