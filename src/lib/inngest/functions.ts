...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (!stored) {
      const pendingEventId = await step.run("retry-pending-notify", async () => {
        ...
        const { data: recent } = await db
          ...
          .limit(5);
        for (const ev of (recent ?? []) as { id: string }[]) {
          const { count } = await db
            .from("notification_deliveries")
            .select("*", { count: "exact", head: true })
            .eq("event_id", ev.id);
          if ((count ?? 0) === 0) return ev.id;
        }
        return null;
      });
      if (pendingEventId) {
        await step.sendEvent("notify-retry", { name: "event/created", data: { event_id: pendingEventId } });
      }
...
async function computeNextSyncAt(
  ...
  for (const row of rows ?? []) {
    const wl = (row as { watchlists?: { organizations?: { refresh_cadence?: string } | { refresh_cadence?: string }[] } | null })
      .watchlists;
