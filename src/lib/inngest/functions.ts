...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (isBaseline) return { changed: true, baseline: true };
...
async function resolveNextSyncAt(
  ...
  for (const row of watchers ?? []) {
    const watchlist = (row as { watchlists?: { organizations?: Pick<Organization, "refresh_cadence"> | Pick<Organization, "refresh_cadence">[] | null } | null }).watchlists;
    if (!watchlist) continue;
    const orgRaw = watchlist.organizations;
    const org = Array.isArray(orgRaw) ? orgRaw[0] : orgRaw;
    if (!org?.refresh_cadence) continue;
    minHours = Math.min(minHours, CADENCE_HOURS[org.refresh_cadence]);
  }
