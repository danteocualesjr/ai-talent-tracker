...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // First snapshot establishes baseline; skip diff/events until a second exists.
    if (priorSnapshotCount <= 1) return { changed: true, eventCreated: false, reason: "first_snapshot" };
