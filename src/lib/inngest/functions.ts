...
async function nextSyncAt(
  ...
  let fastest = cadenceToHours("weekly");
  for (const row of watchers ?? []) {
    ...
    const org = (watchlist as { organizations?: unknown }).organizations;
    const orgRow = Array.isArray(org) ? org[0] : org;
    const cadence = (orgRow as { refresh_cadence?: RefreshCadence } | null)?.refresh_cadence;
    if (cadence) fastest = Math.min(fastest, cadenceToHours(cadence));
  }

  const base =
    profile.status === "left" || profile.status === "stealth" ? Math.min(fastest, 6) : fastest;
