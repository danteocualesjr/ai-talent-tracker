...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // First sync only establishes a baseline — skip event creation.
...
async function nextSyncAt(
  ...
  for (const row of watchers ?? []) {
    const wl = (row as { watchlists?: unknown }).watchlists;
    const watchlist = Array.isArray(wl) ? wl[0] : wl;
    if (!watchlist || typeof watchlist !== "object") continue;
    const orgs = (watchlist as { organizations?: unknown }).organizations;
    const org = Array.isArray(orgs) ? orgs[0] : orgs;
    const cadence = (org as { refresh_cadence?: RefreshCadence } | null)?.refresh_cadence;
    if (cadence) cadences.push(cadence);
  }

  let hours = cadences.length > 0 ? Math.min(...cadences.map(cadenceToHours)) : 24;
