  ...
  async ({ event, step }) => {
    ...
    await step.run("touch-profile", async () => {
      ...
      await db
        .from("profiles")
        .update({
          full_name: merged.full_name ?? profile.full_name,
          headline: merged.headline ?? profile.headline,
          current_company: merged.current_company,
          current_title: merged.current_title,
          location: merged.location ?? profile.location,
          avatar_url: merged.avatar_url ?? profile.avatar_url,
          about: merged.about ?? profile.about,
          github_handle: merged.github_handle ?? profile.github_handle,
          x_handle: merged.x_handle ?? profile.x_handle,
