...
export const refreshProfile = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // First sync establishes baseline — skip event creation on initial snapshot.
...
async function nextSyncAt(
  ...
  for (const w of (watchers ?? []) as Array<{ watchlists?: { organizations?: { refresh_cadence?: RefreshCadence } | { refresh_cadence?: RefreshCadence }[] | null } | { organizations?: { refresh_cadence?: RefreshCadence } | { refresh_cadence?: RefreshCadence }[] | null }[] | null }>) {
  ...
  const fastest = cadences.length > 0 ? Math.min(...cadences.map((c) => CADENCE_HOURS[c])) : 24;
  const base = profile.status === "left" || profile.status === "stealth" ? Math.min(6, fastest) : fastest;
