...
export function safeRedirectPath(next: string | null | undefined, fallback = "/app"): string {
  ...
  if (next.includes("\\") || /[\x00-\x1f\x7f]/.test(next)) return fallback;
  return next;
}

/** Prevent CDATA terminator injection in RSS item fields. */
export function escapeRssCdata(value: string): string {
  return value.replace(/\]\]>/g, "]]]]><![CDATA[>");
}

/** Block SSRF targets in user-configured outbound webhook URLs. */
