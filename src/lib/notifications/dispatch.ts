...
export async function dispatchEvent(eventId: string): Promise<{ dispatched: number }> {
  ...
  for (const ch of (channels ?? []) as NotificationChannel[]) {
    ...
    try {
      ...
      if (prior) {
        await db.from("notification_deliveries").update(row).eq("id", prior.id);
      } else {
      ...
      if (prior) {
        await db.from("notification_deliveries").update({ status: "failed", error: message }).eq("id", prior.id);
      } else {
        await db.from("notification_deliveries").insert({
          channel_id: ch.id,
          event_id: event.id,
