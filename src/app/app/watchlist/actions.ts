...
export async function refreshNowForm(formData: FormData): Promise<void> {
  ...
  const { data: link } = await db
    .from("watchlist_profiles")
    .select("profile_id, watchlists!inner(org_id), profiles(is_opted_out)")
    .eq("profile_id", profileId)
    .eq("watchlists.org_id", org.id)
    .maybeSingle();
  if (!link) return;

  const optedOut = (link as { profiles?: { is_opted_out?: boolean } | { is_opted_out?: boolean }[] }).profiles;
  const isOptedOut = Array.isArray(optedOut) ? optedOut[0]?.is_opted_out : optedOut?.is_opted_out;
