  ...
  return (
    <div className="container max-w-3xl space-y-8 px-4 py-8 md:px-6 md:py-10">
      ...
      <Panel title="Account" bodyClassName="divide-y divide-border/60">
        <dl className="contents">
          <Row label="Email" value={user!.email ?? ""} copy />
...
function Row({
  ...
  const showCopy = Boolean((mono || copy) && value);
  return (
    <div className="group relative flex items-center justify-between gap-3 px-5 py-4 text-sm transition-colors hover:bg-muted/30 focus-within:bg-muted/20">
      ...
      <dd className="flex min-w-0 items-center gap-2">
        <span className={`truncate font-medium transition-colors group-hover:text-foreground ${mono ? "font-mono text-xs text-muted-foreground group-hover:text-foreground/90" : ""} ${capitalize ? "capitalize" : ""}`}>
          {value}
        </span>
