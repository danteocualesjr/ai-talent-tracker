...
export async function addChannel(formData: FormData): Promise<ActionResult> {
  ...
  if (type === "webhook" && org.plan !== "team" && org.plan !== "enterprise") {
    return { error: "Webhooks require a Team or Enterprise plan." };
  }
  ...
  if (type === "email") {
    ...
    if (!r.success) return { error: "Enter a valid Slack webhook URL (https://hooks.slack.com/...)." };
    config = r.data;
