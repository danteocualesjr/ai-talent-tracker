...
import { cdataSafe, siteUrl } from "@/lib/utils";
...
export async function GET() {
  ...
  const items = events.map((e) => {
    ...
    return `
      <item>
        <title><![CDATA[${cdataSafe(`${e.profile.full_name || e.profile.linkedin_handle} — ${e.type.replace(/_/g, " ")}`)}]]></title>
        <link>${link}</link>
        <guid>${link}</guid>
        <pubDate>${new Date(e.detected_at).toUTCString()}</pubDate>
        <description><![CDATA[${cdataSafe(e.summary)}]]></description>
      </item>`;
  }).join("");
