...
async function applySubscription(db: ReturnType<typeof createAdminClient>, sub: Stripe.Subscription) {
  ...
  if (!mapping) throw new Error(`unknown price: ${priceId}`);

  const customerId = sub.customer as string;
  const customer = await stripe.customers.retrieve(customerId);
  const orgId =
    typeof customer === "object" && !customer.deleted ? customer.metadata?.org_id : undefined;
  ...
  const { error } = await db.from("organizations").update(payload).eq("stripe_customer_id", customerId);
  if (error) throw error;
}
