...
export async function POST(req: NextRequest) {
  ...
  try {
    if (
      ...
      if (sub && isBillableSubscription(sub)) await applySubscription(db, sub);
    }
...
function isBillableSubscription(sub: Stripe.Subscription): boolean {
...
async function applySubscription(db: ReturnType<typeof createAdminClient>, sub: Stripe.Subscription) {
  ...
  if (!priceId) {
    throw new Error(`subscription ${sub.id} has no price`);
  }
