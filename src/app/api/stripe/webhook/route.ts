...
async function applySubscription(
  ...
  const orgId = await resolveOrgId(sub, event);
  const payload = {
    plan: mapping.plan,
  ...
  const { error } = await db.from("organizations").update(payload).eq("stripe_customer_id", sub.customer as string);
  if (error) throw error;
}
...
async function resolveOrgId(sub: Stripe.Subscription, event: Stripe.Event): Promise<string | null> {
  if (event.type === "checkout.session.completed") {
    const session = event.data.object as Stripe.Checkout.Session;
