...
async function applySubscription(
  ...
  const { error: byCustomerError } = await db
    .from("organizations")
    .update(update)
    .eq("stripe_customer_id", customerId);
  if (byCustomerError) throw byCustomerError;

  const orgId = orgIdFromSession || sub.metadata?.org_id;
  if (orgId) {
    const { error: byOrgError } = await db.from("organizations").update(update).eq("id", orgId);
