...
async function applySubscription(db: ReturnType<typeof createAdminClient>, sub: Stripe.Subscription) {
  ...
  if (orgId) {
    const { error } = await db.from("organizations").update(update).eq("id", orgId);
    if (error) throw error;
    return;
  }
  ...
  if (!customer.deleted && customer.metadata?.org_id) {
    const { error } = await db.from("organizations").update(update).eq("id", customer.metadata.org_id);
    if (error) throw error;
    return;
  }

  const { error } = await db.from("organizations").update(update).eq("stripe_customer_id", sub.customer as string);
  if (error) throw error;
}
