...
async function loadSubscription(
  event: Stripe.Event,
): Promise<{ sub: Stripe.Subscription | null; orgId?: string }> {
...
async function applySubscription(
  ...
  if (orgIdFromSession) {
    await db.from("organizations").update(updates).eq("id", orgIdFromSession);
    return;
  }

  await db.from("organizations").update(updates).eq("stripe_customer_id", sub.customer as string);
}
