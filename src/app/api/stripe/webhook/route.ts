...
async function applySubscription(db: ReturnType<typeof createAdminClient>, sub: Stripe.Subscription) {
  ...
  if (!mapping) {
    console.error("[stripe/webhook] unmapped price id", priceId, "for subscription", sub.id);
    return;
  }
  ...
  const { error } = await db
    ...
    .eq("stripe_customer_id", sub.customer as string);
  if (error) console.error("[stripe/webhook] failed to update org", error.message);
}
