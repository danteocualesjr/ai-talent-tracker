...
export async function POST(req: NextRequest) {
  ...
  if (
    ...
    if (sub) {
      ...
      if (err) {
        console.error("[stripe webhook] applySubscription failed:", err);
...
async function applySubscription(
  ...
  const { data: byCustomer, error: customerErr } = await db
    ...
    .select("id");
  if (customerErr) return customerErr.message;
  if ((byCustomer ?? []).length > 0) return null;

  const customer = await stripe.customers.retrieve(customerId);
  if (customer.deleted) return "customer deleted";
  const orgId = customer.metadata?.org_id;
  if (!orgId) return "no org matched for customer";
  ...
  if ((byOrg ?? []).length === 0) return "no org matched for customer";
