-- Restrict event reads: public events for everyone, private events for org watchlist members only.
...
drop policy if exists "org events are readable" on public.events;
create policy "org events are readable" on public.events
  for select using (
    profile_id in (
